function [sysErr,sysTime] = dgNavierStokesVIEQNEqn(pde,option,varargin)
%
%   dgNavierStokesVIEQNEqn solve the NS Variational Inequality  equation by DG methods
%
%
%	YcZhang 24/10/2017
%
...
if ~exist('pde','var')
    pde = navierstokesVIEQNData(0,1); % default data
end
...
% Ndof = zeros(maxIt,1);
dgNavierStokesVIEQN_output = cell(maxIt,1);

%% set path and file name
load('setpath_pwd.mat')
dgfunc_name = 'dgNavierStokesVIEQN_';
...
disp('********************  dg NavierStokesVIEQN  ********************')
disp('------------------------------------------------------')
disp('NavierStokesVIEQN Equation:')
...
%% DG methods
disp('------------------------------------------------------')
disp('Solving dgNavierStokesVIEQN: ')
disp('------------------------------------------------------')
% format long e
for n = 1:maxIt
    n_str = num2str(n);
...
%     end
    ...
    disp(['Polygen mesh Nelems: ',num2str(meshInfo.Nelems)])
	%plotPolyMsh(meshInfo)
    %patchPlotMesh(node, elem);

    %% solve equations
    solve_t0 = cputime;
    [Uh, sysInfo]= dgNavierStokesVIEQNSolve(meshInfo,pde,option);
...
%     h_2 = sum(meshInfo.diameters)/length(elem);
    ...
    dgNavierStokesVIEQN_output{n,1} = ...
        struct( ...
...
save(saveFilename, 'dgNavierStokesVIEQN_output');
