function [sysErr,sysTime] = dgTimeTensorStokesEqn(pde,option,varargin)
%
...
if ~exist('pde','var')
    pde = timeTensorStokesData(1); % default data
end
...
dgfunc_name = 'dgTimeTensorStokes_';
...
%% DG methods
disp('------------------------------------------------------')
disp('Solving dgTimeTensorStokes: ')
disp('------------------------------------------------------')
% format long e
for n = 1:maxIt
    n_str = num2str(n);
    ...
    option.p_sigma = option.p_sigma/((1.3)^(n-2));
...
%     elem = elements';
    ...
    %% solve equations
    solve_t0 = cputime;
    [Uh, sysInfo]= dgTimeTensorStokesSolve(meshInfo,pde,option);
