function [sysErr,sysTime] = dgTimeDecoupledSDTEqn2(pde,option,varargin)
%
%   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%   %--------------------------------------------
%       dgTimeDecoupledSDT2, 
...
if ~exist('pde','var')
    pde = TimeDecoupledSDTData2(0,1); % default data
end
...
% Ndof = zeros(maxIt,1);
dgTimeDecoupledSDT2_output = cell(maxIt,1);

%% set path and file name
load('setpath_pwd.mat')
dgfunc_name = 'dgTimeDecoupledSDT2_';
...
disp('********************  dg Time-Stokes-Darcy Transport  ********************')
disp('------------------------------------------------------')
disp('Time-Stokes-Darcy Transport Equation:')
...
%% DG methods
disp('------------------------------------------------------')
disp('Solving dgTimeDecoupledSDT1: ')
disp('------------------------------------------------------')
format long e
for n = 1:maxIt
    close all
...
%     Dnode = vertices'; Delem = elements';
    ...
    %---- mesh 2--- S:[0,1]x[0,1]. D:[0,1]x[-1,0].
	%-------------------- Quad mesh ---------------------
	% get Stokes mesh information
    h_x = 1/2^(n+1); h_y = h_x;
	h_partition=[h_x,h_y];
    ...
    %% solve equations
    solve_t0 = cputime;
    [SD_Uh, Ch, sysInfo]= dgTimeDecoupledSDTSolve2(StokesmeshInfo,DarcymeshInfo,pde,option);
    ...
    dgTimeDecoupledSDT2_output{n,1} = ...
        struct( ...
...
save(saveFilename, 'dgTimeDecoupledSDT2_output');
