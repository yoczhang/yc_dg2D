function [sysErr,sysTime] = dgTensorStokesEqn(pde,option,varargin)
%
...
if ~exist('pde','var')
    pde = tensorStokesData(0,7); % default data
end
...
% 	[node, elem] = generate_Tri_P_T(left,right,bottom,top,h_partition);
    ...
    %% solve equations
    solve_t0 = cputime;
    [Uh, sysInfo]= dgTensorStokesSolve(meshInfo,pde,option);
