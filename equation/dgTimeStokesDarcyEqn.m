function [sysErr,sysTime] = dgTimeStokesDarcyEqn(pde,option,varargin)
%
...
if ~exist('pde','var')
    pde = timeStokesDarcyData(1); % default data
end
...
% Ndof = zeros(maxIt,1);
dgTimeStokesDarcy_output = cell(maxIt,1);

%% set path and file name
load('setpath_pwd.mat')
dgfunc_name = 'dgTimeStokesDarcy_';
...
disp('********************  dg time Stokes Darcy  ********************')
disp('------------------------------------------------------')
disp('time Stokes Darcy Equation:')
...
%% DG methods
disp('------------------------------------------------------')
disp('Solving dgTimeStokesDarcy: ')
disp('------------------------------------------------------')
% format long e
for n = 1:maxIt
    close all
    ...
    if option.usingTensorStokes == 1 && strcmpi(pde.pdecase,'case1')
        %---- mesh 1 --- Stokes: [0,1]x[0,1]; Darcy: [0,1]x[-1,0];
        %-------------------- Tri mesh ---------------------
    ...
    elseif option.usingTensorStokes == 1 && strcmpi(pde.pdecase,'case2')
        %---- mesh 1 --- --- Stokes: [0,1]x[1,2]; Darcy: [0,1]x[0,1];
        %-------------------- Tri mesh ---------------------
        %--- get Stokes mesh information
    ...
    elseif option.usingTensorStokes == 1 && strcmpi(pde.pdecase,'case5')
        %% get Stokes mesh information
    ...
    if option.usingTensorStokes == 1
        [Uh, sysInfo] = dgTimeStokesDarcySolve_usingTensorStokes(StokesmeshInfo,DarcymeshInfo,pde,option);
    end
    ...
    err_t0 = cputime;
    [phih_L2_error(n), phih_H1_error(n)] = dgTimeL2H1Error(sysInfo.terminalT, pde.phi,pde.phix,pde.phiy,phih,DarcymeshInfo,sysInfo.Gaussformulas{1},option.basesType_Pp);
    [uh1_L2_error, uh1_H1_error] = dgTimeL2H1Error(sysInfo.terminalT, pde.u1,pde.u1x,pde.u1y,uh1,StokesmeshInfo,sysInfo.Gaussformulas{1},option.basesType_Fu);
    [uh2_L2_error, uh2_H1_error] = dgTimeL2H1Error(sysInfo.terminalT, pde.u2,pde.u2x,pde.u2y,uh2,StokesmeshInfo,sysInfo.Gaussformulas{1},option.basesType_Fu);
    [ph_L2_error(n), ~] = dgTimeL2H1Error(sysInfo.terminalT, pde.p,pde.px,pde.py,ph,StokesmeshInfo,sysInfo.Gaussformulas{1},option.basesType_Fp);
...
save(saveFilename, 'dgTimeStokesDarcy_output');
