function [sysErr,sysTime] = dgDecoupledSDTransportEqn1(pde,option,varargin)
%
%   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%   %--------------------------------------------
...
if ~exist('pde','var')
    pde = DecoupledSDTransportData1(0,1); % default data
end
...
% Ndof = zeros(maxIt,1);
dgDecoupledSDTransport1_output = cell(maxIt,1);

%% set path and file name
load('setpath_pwd.mat')
dgfunc_name = 'dgDecoupledSDTransport1_';
...
%% DG methods
disp('------------------------------------------------------')
disp('Solving dgDecoupledSDTransport1: ')
disp('------------------------------------------------------')
format long e
for n = 1:maxIt
    close all
...
%     Dnode = vertices'; Delem = elements';
    ...
    StokesmeshInfo.DiffusivityCoeffs = 100*pde.DiffusivityCoeff*DiffusivityCoeffs;
    %----------------------------------------------------------
    %-------------- Darcy meshInfo ----------------------
    ...
    %% solve equations
    solve_t0 = cputime;
    [SD_Uh, Ch, sysInfo]= dgDecoupledSDTransportSolve1(StokesmeshInfo,DarcymeshInfo,pde,option);
    ...
    dgDecoupledSDTransport1_output{n,1} = ...
        struct( ...
...
save(saveFilename, 'dgDecoupledSDTransport1_output');
