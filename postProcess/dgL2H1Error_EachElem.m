function [L2err, H1err] = dgL2H1Error_EachElem(P,Px,Py,Ph,meshInfo,formulaGauss2D,basesType_trial)
%
...
%-------------------------------------------------------------------------------------------------------------------%
    for nt = 1:(size(coordTri0Elem,1)/3)
        ...
        valuePx = Px(phyGpoints(:,1), phyGpoints(:,2)); % [Npoints x 1]
        valuePy = Py(phyGpoints(:,1), phyGpoints(:,2)); % [Npoints x 1]
        
        L2errElem(ii) = L2errElem(ii) + phyGweights'*(valueP - trialPb*PhOnElem).^2;
        H1errElem(ii) = H1errElem(ii) + phyGweights'*(valuePx-trialPbx*PhOnElem).^2 ...
