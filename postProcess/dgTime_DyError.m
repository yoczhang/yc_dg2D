function Dy_err = dgTime_DyError(t,P,Px,Py,Ph,meshInfo,formulaGauss2D,basesType_trial)
%
...
%-------------------------------------------------------------------------------------------------------------------%
    for nt = 1:(size(coordTri0Elem,1)/3)
        ...
        Dy_err = Dy_err + phyGweights'*(valuePy-trialPby*PhOnElem).^2;
    end % for nt

end % for ii
Dy_err = sqrt(Dy_err);
end % function dgL2Div_error
