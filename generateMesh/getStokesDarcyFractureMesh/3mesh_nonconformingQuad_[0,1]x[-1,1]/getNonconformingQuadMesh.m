...
for nn = 1:4
    ...
    meshName = ['nonconformingQuad_[0,1]x[1,2]_',meshN];
...
%     plotGrid(D_temp1, 'FaceColor', 'none'); axis equal; axis off
    %-
    ...
    D_G = translateGrid(D_G,[0,-1]);
%     plotGrid(D_G, 'FaceColor', 'none'); axis equal; axis on
    ...
    if doplot
        ...
        %--- plot interface
        %- plot Stokes-Darcy interface
        plot([0,1],[0,0],'-r', 'LineWidth',1.2)

        %- plot Darcy-Fracture interface
        ...
        axis equal
        if nn == 1
            print('nonconformingQuad.jpg','-djpeg', '-r750')
            print('nonconformingQuad.eps','-depsc')
        end
    end % if
