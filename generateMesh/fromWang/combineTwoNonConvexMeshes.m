function [node, elem] = combineTwoNonConvexMeshes(refine)

%% Two meshes
[node1, elem1] = squarequadmesh([0,1,1,2], 1/2^refine);
% for i = 1:refine, [node1, elem1] = uniformrefinequad(node1, elem1); end
[node1,elem1] = non_convex_octagona_mesh(node1,elem1);

[node2, elem2] = squarequadmesh([0,1,0,1], 1/2^refine);
% for i = 1:refine, [node2, elem2] = uniformrefinequad(node2, elem2); end
[node2,elem2] = non_convex_octagona_mesh(node2,elem2);
...
%% Plot
% figure; PolyMshr_PlotMsh(node, elem, length(elem));
