function [node, elem] = combineTwoCentroidalVoronoiMeshes(meshfile)

% The 1st part is Stokes, and the 2nd part is Darcy domain.

%% Two meshes
mesh = load(meshfile);
node2 = mesh.node; elem2 = mesh.elem;

node1 = node2; node1(:,2) = 2 - node1(:,2);
elem1 = cell(length(elem2),1);
for n = 1:length(elem2)
    localIndex = elem2{n};
    elem1{n} = localIndex(end:-1:1);
end
...
elem = cell(NT1+NT2,1); 
% elem(1:NT1) = elem1; elem(NT1+1:end) = elem2;

for n = 1:length(elem1)
    verts = elem1{n};
