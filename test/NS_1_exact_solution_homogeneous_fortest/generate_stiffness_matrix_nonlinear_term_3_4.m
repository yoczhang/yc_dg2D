function [r_interE, r_boundaryE]=generate_stiffness_matrix_nonlinear_term_3_4(DGT,DGE,DG_edge_flag,fun_name_2,fun_name_3,fun_name_4,fun_name_5,basis_type,u1_h,u2_h...
                                                       ,interior_edges,boundary_edges,Gauss_coefficient_reference_1D,Gauss_point_reference_1D)

N_loc=(basis_type+1)*(basis_type+2)/2;
r_interE=sparse(size(DGT,2)*N_loc,size(DGT,2)*N_loc);
r_boundaryE=sparse(size(DGT,2)*N_loc,size(DGT,2)*N_loc);
...
for i=1:size(edge_brother,2)
    ...
    for alpha=1:N_loc
        for beta=1:N_loc
            r_interE((ele_large-1)*N_loc+beta,(ele_large-1)*N_loc+alpha)=r_interE((ele_large-1)*N_loc+beta,(ele_large-1)*N_loc+alpha)+b_1(alpha,beta);
            r_interE((ele_small-1)*N_loc+beta,(ele_small-1)*N_loc+alpha)=r_interE((ele_small-1)*N_loc+beta,(ele_small-1)*N_loc+alpha)+b_1(alpha,beta);
            r_interE((test_ele-1)*N_loc+beta,(ele_small-1)*N_loc+alpha)=r_interE((test_ele-1)*N_loc+beta,(ele_small-1)*N_loc+alpha)+b_2(alpha,beta);
            r_interE((test_ele-1)*N_loc+beta,(ele_large-1)*N_loc+alpha)=r_interE((test_ele-1)*N_loc+beta,(ele_large-1)*N_loc+alpha)-b_2(alpha,beta);
        end
    end
end
...
for i=1:length(boundary_edges)
    ...
    elseif begin_point(2)==end_point(2) % horizontal edge
        ...
        for k=1:length(Gauss_coefficient_local_1D)
            x=Gauss_point_local_1D(k);y=slope*(x-begin_point(1))+begin_point(2);
            b_1=b_1+Gauss_coefficient_local_1D(k)*Jacobi...
                *feval(fun_name_4,x,y,solution_1_ele,solution_2_ele,basis_type,normal);
            b_2=b_2+Gauss_coefficient_local_1D(k)*Jacobi...
                *feval(fun_name_5,x,y,solution_1_ele,solution_2_ele,basis_type,normal);
        end
    end
    
    for alpha=1:N_loc
        for beta=1:N_loc
            r_boundaryE((ele-1)*N_loc+beta,(ele-1)*N_loc+alpha)=r_boundaryE((ele-1)*N_loc+beta,(ele-1)*N_loc+alpha)+b_1(alpha,beta);
            if value<=0
                r_boundaryE((ele-1)*N_loc+beta,(ele-1)*N_loc+alpha)=r_boundaryE((ele-1)*N_loc+beta,(ele-1)*N_loc+alpha)+b_2(alpha,beta);
            end
        end
    end
end    
