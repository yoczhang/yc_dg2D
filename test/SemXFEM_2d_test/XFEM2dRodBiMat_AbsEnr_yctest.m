...
%--------------------------------------------
function XFEM2dRodBiMat_AbsEnr_yctest
...
xx = node(:,1); yy = node(:,2);
ElemNum = size(Mesh,1);
yc_meshInfo = polyMeshAuxStructure(node, Mesh);
plotPolyMsh(yc_meshInfo)
%------------------------------------
aa=1; bb=0; cc=-xx(9); % Vertical disc. at x=0.5.
ff = GetLevelSet(aa, bb, cc, xx, yy);
PlotLevelSet_yctest(Mesh, xx, yy, ff, ElemNum)
