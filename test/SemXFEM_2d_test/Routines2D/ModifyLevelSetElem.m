...
% nodes together with their level-set value build a plane. That is, 
% (r1, s1, f1), (r2, s2, f2), (r3, s3, f3), (r4, s4, f4) lie on a 
% plane where fi are the level-set values.
...
function [CutEdge1, CutEdge2] = GetCutEdges(ffElem)
    ...
    Count = 0;
    if SignVect(1) ~= SignVect(2) % Disc. is between node 1 and 2.
        Count = Count + 1;
        CutEdge(Count, 1) = 1;
    end 
