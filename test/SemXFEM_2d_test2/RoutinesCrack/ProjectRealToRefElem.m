...
% Project current integration point into [-1,1] x [-1,1] reference element.
% Note: This routine does not consider all special cases where the transformation
% between the real and reference element is singular with the general procedure.

x1 = xElem(1); x2 = xElem(2); x3 = xElem(3); x4 = xElem(4); 
y1 = yElem(1); y2 = yElem(2); y3 = yElem(3); y4 = yElem(4); 
...
else % Transform all others like this.
    px = x1 + x2 + x3 + x4;
    ux =-x1 + x2 + x3 - x4;
    vx = x1 - x2 + x3 - x4;
    tx =-x1 - x2 + x3 + x4;
