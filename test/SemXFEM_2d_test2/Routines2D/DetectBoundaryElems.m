...
% Construct 1D boundary element data. Therefore, the element edges
% on the boundary are extracted. ElemNum1D is the number of 1D elements,
% BoundaryElems1D stores the element nodes corresponding to the global
% 2D mesh, and CorrBoundaryElems2D stores which 2D element belongs to
...
% % Plot situation.
% for i = 1 : count
%     Corr2DElem = CorrBoundaryElems2D(i, 1);
%     patch(xx(Mesh(Corr2DElem, :)), yy(Mesh(Corr2DElem, :)), 0.01*[1 1 1 1], 'r')    
%     Mesh1D = BoundaryElems1D(i, :);
%     a = line(xx(Mesh1D), yy(Mesh1D), [1 1]);
%     set(a, 'Color', 'b', 'LineWidth', 3)
% end
% error('STOPPED INSIDE!')
