function [matDirichletEdge, rhs_gD] = ...
    matDirichletEdgeAverJumpSDTransport_Poisson(Coeffs_func,func_gD,meshInfo, option,formulaGauss1D,degreek_trial,degreek_test)
%
%   %---------------------------------------------------------------------
...
for ii = 1:length(DirichletEdges)
    ...
    elemP_value_DC = elemP_DC * ones(length(phyGweights),1); % [Npoints x 1]
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    r = elemP_nu(1) * (elemP_value_DC.*elemP_testPbx)' *  (valueCoeff_k11.*valuefunc_gD.*phyGweights) ...
        + elemP_nu(2) * (elemP_value_DC.*elemP_testPby)' * (valueCoeff_k22.*valuefunc_gD.*phyGweights);
...
%
    ...
    A1_pp = elemP_nu(1) * elemP_testPb' * bsxfun(@times,elemP_value_DC.*valueCoeff_1.*phyGaussweights,elemP_trialPbx) ...
    ...
    A2_pp = elemP_nu(1) * (elemP_value_DC.*elemP_testPbx)' * bsxfun(@times,valueCoeff_1.*phyGaussweights,elemP_trialPb) ...
