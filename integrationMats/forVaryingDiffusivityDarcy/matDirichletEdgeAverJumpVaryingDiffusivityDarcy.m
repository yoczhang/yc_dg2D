function [matDirichletEdge, rhs_gD] = ...
    matDirichletEdgeAverJumpVaryingDiffusivityDarcy(Coeffs_func,func_gD,meshInfo, option,formulaGauss1D,degreek_trial,degreek_test)
%
%   %----------------------------------------
%       modified from: matDirichletEdgeAverJumpPoisson.m
%   %----------------------------------------
%
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    if abs(meshInfo.baryEdge(DirichletEindx,1)-0) <= 5e-8
        [rhsDirRow, ~] = getROWandCOL(elemP, 0, 1, NTbases_test);

        r = elemP_nu(1) * (elemP_value_DC.*eleP_testPbx)' *  (valueCoeff_k11.*valuefunc_gD.*phyGweights) ...
            + elemP_nu(2) * (elemP_value_DC.*eleP_testPby)' * (valueCoeff_k22.*valuefunc_gD.*phyGweights);
