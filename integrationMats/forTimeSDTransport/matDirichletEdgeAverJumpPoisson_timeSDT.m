function [matDirichletEdge, rhs_gD] = ...
    matDirichletEdgeAverJumpPoisson_timeSDT(Coeffs_func,func_gD,meshInfo, option,formulaGauss1D,degreek_trial,degreek_test)
%
%   %-----------------------------------------
%       Just copy from matDirichletEdgeAverJumpPoisson_timeSD.m
%   %-----------------------------------------
%
...
for ii = 1:length(DirichletEdges)
    ...
    valuefunc_gD = func_gD(0,phyGpointsX, phyGpointsY); % [Npoints x 1]
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    r = elemP_nu(1) * elemP_testPbx' *  (elemP_value_Permeability.*valueCoeff_k11.*valuefunc_gD.*phyGweights) ...
        + elemP_nu(2) * elemP_testPby' * (elemP_value_Permeability.*valueCoeff_k22.*valuefunc_gD.*phyGweights);
