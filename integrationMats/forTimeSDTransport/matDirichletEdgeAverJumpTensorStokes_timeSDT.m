function [matDirichletEdge_u1v1,matDirichletEdge_u1v2,matDirichletEdge_u2v1,matDirichletEdge_u2v2,matDirichletEdge_pv1,matDirichletEdge_pv2,rhs_gD1,rhs_gD2,rhs_div] = ...
    matDirichletEdgeAverJumpTensorStokes_timeSDT(Coeffs_func, func_gD1, func_gD2, meshInfo, option, formulaGauss1D, degreek_u, degreek_p)
%
%   %-----------------------------------------
%       Just copy from matDirichletEdgeAverJumpTensorStokes_timeSD.m
%   %-----------------------------------------
%
...
matDirichletEdge_u1v1 = sparse(Nelems*Nbases_u,Nelems*Nbases_u);
matDirichletEdge_u1v2 = sparse(Nelems*Nbases_u,Nelems*Nbases_u);
matDirichletEdge_u2v1 = sparse(Nelems*Nbases_u,Nelems*Nbases_u);
matDirichletEdge_u2v2 = sparse(Nelems*Nbases_u,Nelems*Nbases_u);
matDirichletEdge_pv1 = sparse(Nelems*Nbases_u,Nelems*Nbases_p);
matDirichletEdge_pv2 = sparse(Nelems*Nbases_u,Nelems*Nbases_p);
...
for ii = 1:length(DirichletEdges)
    ...
    valuefunc_gD1 = func_gD1(0,phyGpointsX, phyGpointsY); % [Npoints x 1]
    valuefunc_gD2 = func_gD2(0,phyGpointsX, phyGpointsY); % [Npoints x 1]
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    [uv_Row, uv_Col] = getROWandCOL(elemP, 0, Nbases_u, Nbases_u);
    ...
    % get the position in the big mat
    [qv_Row, qv_Col] = getROWandCOL(elemP, 0, Nbases_p, Nbases_u);
    ...
    matDirichletEdge_pv1 = matDirichletEdge_pv1 ...
        + sparse(qv_Row(:), qv_Col(:), qv_A1(:), Nelems*Nbases_u, Nelems*Nbases_p);
    matDirichletEdge_pv2 = matDirichletEdge_pv2 ...
        + sparse(qv_Row(:), qv_Col(:), qv_A2(:), Nelems*Nbases_u, Nelems*Nbases_p);
    ...
    r1 = elemP_nu(1) * u_elemP_Pbx' *  (valueCoeff_k11.*valuefunc_gD1.*phyGweights) ...
        + 0.5 * elemP_nu(2) * u_elemP_Pby' *  (valueCoeff_k11.*valuefunc_gD1.*phyGweights) ...
        + 0.5 * elemP_nu(1) * u_elemP_Pby' *  (valueCoeff_k11.*valuefunc_gD2.*phyGweights);
        %> [NTbases_test x Npoints] * [Npoints x 1] = [NTbases_test x 1].
    r1 = p_epsilon*2*r1 + (p_sigma/areaEdge^p_beta)*u_elemP_Pb' * (valuefunc_gD1.*phyGweights);
    rhs_gD1(rhsDirRow,1) = rhs_gD1(rhsDirRow,1) + r1;
    
    r2 = 0.5 * elemP_nu(2) * u_elemP_Pbx' *  (valueCoeff_k11.*valuefunc_gD1.*phyGweights) ...
        + 0.5 * elemP_nu(1) * u_elemP_Pbx' *  (valueCoeff_k11.*valuefunc_gD2.*phyGweights) ...
        + elemP_nu(2) * u_elemP_Pby' *  (valueCoeff_k11.*valuefunc_gD2.*phyGweights);
        %> [NTbases_test x Npoints] * [Npoints x 1] = [NTbases_test x 1].
    r2 = p_epsilon*2*r2 + (p_sigma/areaEdge^p_beta)*u_elemP_Pb' * (valuefunc_gD2.*phyGweights);
