function [matElemDuDv, vecRhsF] = ...
    matElemCoeffsDtrialDtestSDTransport2_Poisson(Coeffs_func, func_f, meshInfo, formulaGauss2D, degreek_trial, degreek_test)
%
%   %---------------------------------------------------------------------
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    for nt = 1:(size(coordTri0Elem,1)/3)
        ...
        %-- MatElem
        TrialTest_temp = TrialTest_temp + getMatOnElem(elem_value_Permeability.*valueCoeff_1, trialPb, testPb, phyGweights);
