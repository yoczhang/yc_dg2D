function rhs_gDt = ...
    vecRhsDirichletEdgeSDTransport2_Transport( t, ...
    Coeffs_func, func_gD, DarcymeshInfo, StokesmeshInfo, ...
    option, formulaGauss1D, degreek_testC)
%
%   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%   %--------------------------------------------
...
%
%   %-------------------------------------------------------------------
%       Just copy form vecEdgeRhsDirichletTransport.m
%   %--------------------------------------------------------------------
%
...
rhs_gDt = zeros(Nelems*C_Nbases_test,1);
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    [rhsDirRow, ~] = getROWandCOL(elemP, 0, 1, C_Nbases_test);
    
    elemP_valueDarcy_DC = elemP_Darcy_DC * ones(length(phyGweights),1);
    
    r = elemP_nu(1) * elemP_testPbx' *  (elemP_valueDarcy_DC.*valueCoeff_k11.*valuefunc_gD.*phyGweights) ...
        + elemP_nu(2) * elemP_testPby' * (elemP_valueDarcy_DC.*valueCoeff_k22.*valuefunc_gD.*phyGweights);
        %> [NTbases_test x Npoints] * [Npoints x 1] = [NTbases_test x 1].
    r = p_epsilon_c*r + (p_sigma_c/areaEdge^p_beta_c)*elemP_testPb' * (valuefunc_gD.*phyGweights);
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    %-- subPart -- the rhs Dirichlet edges
    %-- get the rhs Dirchlet vector
    [rhsDirRow, ~] = getROWandCOL(D_Nelems + elemP, 0, 1, C_Nbases_test);
    
    elemP_valueStokes_DC = elemP_Stokes_DC * ones(length(phyGweights),1);
    
    r = elemP_nu(1) * elemP_testPbx' *  (elemP_valueStokes_DC.*valueCoeff_k11.*valuefunc_gD.*phyGweights) ...
        + elemP_nu(2) * elemP_testPby' * (elemP_valueStokes_DC.*valueCoeff_k22.*valuefunc_gD.*phyGweights);
        %> [NTbases_test x Npoints] * [Npoints x 1] = [NTbases_test x 1].
    r = p_epsilon_c*r + (p_sigma_c/areaEdge^p_beta_c)*elemP_testPb' * (valuefunc_gD.*phyGweights);
