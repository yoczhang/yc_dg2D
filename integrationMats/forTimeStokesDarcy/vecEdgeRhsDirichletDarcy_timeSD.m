function rhs_gD1 = ...
    vecEdgeRhsDirichletDarcy_timeSD(t, Coeffs_func, func_gD1, meshInfo, option, formulaGauss1D, degreek_test)
%
...
rhs_gD1 = zeros(Nelems*Nbases_test,1);
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    [rhsDirRow, ~] = getROWandCOL(elemP, 0, 1, Nbases_test);
    
    r1 = elemP_nu(1) * (elemP_value_Permeability.*elemP_testPbx)' *  (valueCoeff_k11.*valuefunc_gD1.*phyGweights) ...
        + elemP_nu(2) * (elemP_value_Permeability.*elemP_testPby)' * (valueCoeff_k22.*valuefunc_gD1.*phyGweights);
        %> [NTbases_test x Npoints] * [Npoints x 1] = [NTbases_test x 1].
    r1 = p_epsilon*r1 + (p_sigma/areaEdge^p_beta)*elemP_testPb' * (valuefunc_gD1.*phyGweights);
