function vecRhsFt = vecElemRhsFtTransport(t, func_f, meshInfo, formulaGauss2D, degreek_test)
%
%   %--------------------------------------------------
%       Just copy from vecElemRhsFtParabolic.m
%   %--------------------------------------------------
%
...
vecRhsFt = zeros(Nelems*NTbases_test,1);
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    for nt = 1:(size(coordTri0Elem,1)/3)
        ...
        [testPb, ~, ~] = localBases2D(ele_xT, ele_yT, ele_hT, phyGpoints(:,1), phyGpoints(:,2), degreek_test);
