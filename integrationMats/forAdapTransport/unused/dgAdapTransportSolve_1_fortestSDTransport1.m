function [Uh, sysInfo] = dgAdapTransportSolve_1_fortestSDTransport1(meshInfo,pde,option)
%
%   %---------------------------------------
%       dgAdapTransportSolve_1
%       This test case from: Discontinuous Galerkin methods for
%       convection-diffusion equations for varying and vanishing
...
[M, D_elem_dudv, C_elem_udv, rhs0] = ...
    matElemCoeffsDtrialDtestAdapTransport_fortestSDTransport1(Coeffs_func,pde.u0,meshInfo,Gaussformulas{1}, trial_k, test_k);

if option.Adap == 1
    [D_interE, D_upwindE, C_interE_upwind] = ...
        matInterEdgeAverJumpAdapTransport_fortestSDTransport1(Coeffs_func, meshInfo,option,Gaussformulas{3},trial_k,test_k);
end


if strcmpi(bd_case,'allDirichlet')
    [D_matDirichletEdge, C_matDirichletEdge] = ...
        matDirichletEdgeAverJumpAdapTransport_fortestSDTransport1(Coeffs_func, meshInfo, option, Gaussformulas{3}, trial_k, test_k);
...
save ref_Matrix1 M D_elem_dudv C_elem_udv rhs0 D_interE D_upwindE C_interE_upwind D_matDirichletEdge C_matDirichletEdge A
...
for n = 0:NT-1
    ...
    rhs_ft = vecElemRhsFtAdapTransport_fortestSDTransport1(t, pde.f_rhs, meshInfo, Gaussformulas{1}, test_k);
    
    if strcmpi(bd_case,'allDirichlet')
        rhs_DiriBC = vecEdgeRhsDirichletAdapTransport_fortestSDTransport1(t, Coeffs_func, pde.gD, ...
    ...
    Uh = sysM\Rhs;
    ref_rhs_ft = rhs_ft; 
    ref_rhs_DiriBC = rhs_DiriBC;
    ref_Rhs = Rhs;
    ref_Uh = Uh;
    save ref_Matrix2 ref_rhs_ft ref_rhs_DiriBC ref_Rhs ref_Uh
...
if strcmpi(bd_case,'haveInflow')
    ...
    inflowEdgeIndex_1 = bdEdgeIndex( abs(meshInfo.baryEdge(bdEdgeIndex,1)-0) <= 5e-8 );
    inflowEdgeIndex = inflowEdgeIndex_1;
    
%     inflowEdgeIndex_1 = bdEdgeIndex( abs(meshInfo.baryEdge(bdEdgeIndex,1)-0) <= 5e-8 );
%     inflowEdgeIndex_2 = bdEdgeIndex( abs(meshInfo.baryEdge(bdEdgeIndex,2)-0) <= 5e-8 );
%     inflowEdgeIndex = union(inflowEdgeIndex_1,inflowEdgeIndex_2);
    
    outflowEdgeIndex_1 = bdEdgeIndex( abs(meshInfo.baryEdge(bdEdgeIndex,1)-2) <= 5e-8 );
    outflowEdgeIndex = outflowEdgeIndex_1;
    
%     outflowEdgeIndex_1 = bdEdgeIndex( abs(meshInfo.baryEdge(bdEdgeIndex,1)-2) <= 5e-8 );
%     outflowEdgeIndex_2 = bdEdgeIndex( abs(meshInfo.baryEdge(bdEdgeIndex,2)-1) <= 5e-8 );
%     outflowEdgeIndex = union(outflowEdgeIndex_1,outflowEdgeIndex_2);
