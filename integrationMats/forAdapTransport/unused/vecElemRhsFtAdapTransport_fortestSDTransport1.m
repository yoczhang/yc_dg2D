function vecRhs_f = vecElemRhsFtAdapTransport_fortestSDTransport1(t, func_f, meshInfo, formulaGauss2D, degreek_test)
%
%   %--------------------------------------------------
...
vecRhs_f = zeros(Nelems*Nbases_test,1);
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    for nt = 1:(size(coordTri0Elem,1)/3)
        ...
        [testPb, ~, ~] = localBases2D(elem_xT, elem_yT, elem_hT, phyGpoints(:,1), phyGpoints(:,2), degreek_test);
            %> testPb, testPbx, testPby, [Npoints x NTbases_test].
        
        %--- the funcValue may be chosen by case.
%         valuefunc_f = func_f(t, phyGpoints(:,1), phyGpoints(:,2));
        valuefunc_f = func_f(t, phyGpoints(:,1), phyGpoints(:,2),elem_flag);
    ...
    Row_vec = ((ii_twoDomain-1)*Nbases_test+1 : ii_twoDomain*Nbases_test);
    vecRhs_f(Row_vec,1) = vecRhs_f(Row_vec,1) + rhs_f_temp;
