function [Uh, sysInfo] = dgAdapTransportSolve_1_fortestSDTransport(meshInfo,pde,option)
%
%   %---------------------------------------
...
[M, D_elem_dudv, C_elem_udv, rhs0] = ...
    matElemCoeffsDtrialDtestAdapTransport_fortestSDTransport(Coeffs_func,pde.u0,meshInfo,Gaussformulas{1}, trial_k, test_k);
% matElemCoeffsDtrialDtestAdapTransport
if option.Adap == 1
    [D_interE, D_upwindE, C_interE_upwind] = ...
        matInterEdgeAverJumpAdapTransport_fortestSDTransport(Coeffs_func, meshInfo,option,Gaussformulas{3},trial_k,test_k);
%     matInterEdgeAverJumpAdapTransport
else
    [D_interE, D_upwindE, C_interE_upwind] = ...
...
if strcmpi(bd_case,'allDirichlet')
    [D_matDirichletEdge, C_matDirichletEdge] = ...
        matDirichletEdgeAverJumpAdapTransport_fortestSDTransport(Coeffs_func, meshInfo, option, Gaussformulas{3}, trial_k, test_k);
%     matDirichletEdgeAverJumpAdapTransport
...
for n = 0:NT-1
    ...
    rhs_ft = vecElemRhsFtAdapTransport_fortestSDTransport(t, pde.f_rhs, meshInfo, Gaussformulas{1}, test_k);
    
    if strcmpi(bd_case,'allDirichlet')
        rhs_DiriBC = vecEdgeRhsDirichletAdapTransport_fortestSDTransport(t, Coeffs_func, pde.gD, ...
