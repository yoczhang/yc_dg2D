function [matInterEdge, matDiffusiveAdapEdge, matConvectionUpwindEdge] = ...
    matInterEdgeAverJumpAdapTransport_noAdap(Coeffs_func, meshInfo, option,formulaGauss1D,degreek_trial,degreek_test)
%
%
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    if 0 && (elemP_DC ~= elemM_DC) && (value_vector_u_1(1)*H_elem_nu(1)+value_vector_u_2(1)*H_elem_nu(2) > 0)
        theta_DC = H_DC/P_DC;
        ...
        matDiffusiveAdapEdge = matDiffusiveAdapEdge ...
            ...
            + (p_sigma1/areaEdge^p_beta)*( (1-theta_DC)*Adap_A3_up(:) + theta_DC*uv_A3(:) ), ...
            Nelems*NTbases_test, Nelems*NTbases_trial);
        
        
        % the convection upwind term
        ConvectionUpwind_mat = getMatOnConvectionUpwindEdgeHP(phyGweights, ...
            H_elem, elemP, ...
            value_vector_u_1, ...
            value_vector_u_2, ...
            elemP_nu, ...
...
% %             Nelems*NTbases_test, Nelems*NTbases_trial);
% %     else
% %         max_DC = max(elemP_DC, elemM_DC); % get the larger Diffusivity coefficients
% %         min_DC = min(elemP_DC, elemM_DC);
% %         theta_DC = min_DC/max_DC;
% %         
% %         [Adap_A1_up, Adap_A2_up, Adap_A3_up] = ...
% %             getMatOnDiffusiveAdapEdge_up(phyGweights, ...
% %             value_vector_u_1, value_vector_u_2, ...
% %             elemP_value_DC, elemM_value_DC, ...
% %             elemP_nu, ...
% %             elemP_trialPb, elemP_trialPbx, elemP_trialPby, ...
% %             elemP_testPb, elemP_testPbx, elemP_testPby, ...
