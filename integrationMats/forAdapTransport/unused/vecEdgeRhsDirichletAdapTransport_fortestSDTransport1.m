function rhs_gDt = ...
    vecEdgeRhsDirichletAdapTransport_fortestSDTransport1(t, Coeffs_func, func_gD, meshInfo, option, formulaGauss1D, degreek_test)
%
%   %-------------------------------------------------------------------
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    [rhsDirRow, ~] = getROWandCOL(elemP_twoDomain, 0, 1, NTbases_test);
    
    r = elemP_nu(1) * elemP_testPbx' *  (elemP_value_DC.*valueCoeff_k11.*valuefunc_gD.*phyGweights) ...
        + elemP_nu(2) * elemP_testPby' * (elemP_value_DC.*valueCoeff_k22.*valuefunc_gD.*phyGweights);
