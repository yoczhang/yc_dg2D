function [Krhs_gN1,Krhs_gN2] = ...
    vecNeumannEdgeAverJumpNavierStokes2(pde, meshInfo, formulaGauss1D, degreek_u)
%
%   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%   %--------------------------------------------
...
NeumannEdges = meshInfo.NeumannEdgeIndex; % here we need the DirichletEdges information.

% setting
NTbases_u = (degreek_u+1)*(degreek_u+2)/2;
mu = pde.mu;

Krhs_gN1 = zeros(Nelems*NTbases_u,1);
Krhs_gN2 = zeros(Nelems*NTbases_u,1);

for ii = 1:length(NeumannEdges)
    ...
    NeumannEindx = NeumannEdges(ii); 
    ePoint1 = meshInfo.node(meshInfo.edge(NeumannEindx,1),:); % [1 x 2], the (x-coord, y-coord) of the first point of ii-th edge.
    ePoint2 = meshInfo.node(meshInfo.edge(NeumannEindx,2),:); % [1 x 2], the (x-coord, y-coord) of the second point of ii-th edge.
    ...
    phyGweights = meshInfo.areaEdge(NeumannEindx) * formulaGauss1D(:,2); 
        %> [Npoints x 1], the physical Gauss weights on ii-th edge.
    
    elem1 = meshInfo.edge2elem(NeumannEindx,1);
    elem2 = meshInfo.edge2elem(NeumannEindx,2);
    local_e1 = meshInfo.edge2elem(NeumannEindx,3);
    local_e2 = meshInfo.edge2elem(NeumannEindx,4);
    ...
    valuefunc_u1x = pde.u1x(phyGpointsX, phyGpointsY); % [Npoints x 1]
    valuefunc_u1y = pde.u1y(phyGpointsX, phyGpointsY); % [Npoints x 1]
    valuefunc_u2x = pde.u2x(phyGpointsX, phyGpointsY); % [Npoints x 1]
    valuefunc_u2y = pde.u2y(phyGpointsX, phyGpointsY); % [Npoints x 1]
    valuefunc_p = pde.p(phyGpointsX, phyGpointsY); % [Npoints x 1]
    valuefunc_mu = mu(phyGpointsX, phyGpointsY);
...
%     [p_elemP_Pb, ~, ~] = ...
%         localBases2D(elemP_xT, elemP_yT, elemP_hT, phyGpointsX, phyGpointsY, degreek_u);
%         %> p_eleP_Pb, p_eleP_Pbx, p_eleP_Pby, [Npoints x NTbases_p]
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    [rhsNeuRow, ~] = getROWandCOL(elemP, 0, 1, NTbases_u);
    
% %     K_gN1_temp = u_elemP_Pb'*( phyGweights.*valuefunc_u1x*elemP_nu(1) ...
% %         +phyGweights.*valuefunc_u1y*elemP_nu(2) ); % [Nbases x 1]
% %     K_gN2_temp = u_elemP_Pb'*( phyGweights.*valuefunc_u2x*elemP_nu(1) ...
% %         +phyGweights.*valuefunc_u2y*elemP_nu(2) ); % [Nbases x 1]
    
    K_gN1_temp = u_elemP_Pb'*(phyGweights.*( ...
        (2*valuefunc_mu.*valuefunc_u1x-valuefunc_p)*elemP_nu(1) ...
        + valuefunc_mu.*(valuefunc_u1y+valuefunc_u2x)*elemP_nu(2) ...
        ) ); % [Nbases x 1]
    K_gN2_temp = u_elemP_Pb'*(phyGweights.*( ...
        valuefunc_mu.*(valuefunc_u1y+valuefunc_u2x)*elemP_nu(1) ...
        + (2*valuefunc_mu.*valuefunc_u2y-valuefunc_p)*elemP_nu(2)  ...
        ) ); % [Nbases x 1]

    Krhs_gN1(rhsNeuRow,1) = Krhs_gN1(rhsNeuRow,1) + ( K_gN1_temp );
    Krhs_gN2(rhsNeuRow,1) = Krhs_gN2(rhsNeuRow,1) + ( K_gN2_temp );
