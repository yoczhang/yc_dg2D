function [Uh, sysInfo] = mdgTimeDarcySolve_test(meshInfo,pde,option)
%
...
meshInfo = ...
    getBoundaryInfo(meshInfo, bd_case);
...
% tensor Stokes
[elem_u0v0,elem_p0q0,elem_DuDv,elem_pv1,elem_pv2,rhs_u10,rhs_u20,rhs_p0,lambda_p] = ...
    matElemDuDvMDGTimeDarcy(Coeffs_func,pde.u10,pde.u20, pde.p0, meshInfo,Gaussformulas{1}, degree_u, degree_p);

[Inter_u1v1, Inter_pv1, Inter_pv2, Inter_pq] = ...
    InterEdgeAverJumpMDGTimeDarcy(Coeffs_func, meshInfo, option, formulaGauss1D, degree_u, degree_p);

[Diri_u1v1,Diri_pv1,Diri_pv2,~,~,~] = ...
    DirEdgeMDGTimeDarcy(Coeffs_func, pde.gD1, pde.gD2, meshInfo, option, formulaGauss1D, degree_u, degree_p);

% the Stokes system matrix and rhs
% Stokes_stiff = [mu*(elem_u0v0)+Inter_u1v1,      zeroUU,     -elem_pv1+Inter_pv1,     zeros(dof_u1,1);
%     zeroUU,     mu*(elem_u0v0)+Inter_u1v1,      -elem_pv2+Inter_pv2,     zeros(dof_u1,1);
%     -(-elem_pv1+Inter_pv1)',     -(-elem_pv2+Inter_pv2)',    0 * Inter_pq,     lambda_p;
%     zeros(1, dof_u1),       zeros(1, dof_u1),       lambda_p',      0];

% Stokes_stiff = [mu*(elem_u0v0)+Inter_u1v1+Diri_u1v1,      zeroUU,     -elem_pv1+Inter_pv1 + Diri_pv1,     zeros(dof_u1,1);
%     zeroUU,     mu*(elem_u0v0)+Inter_u1v1+Diri_u1v1,      -elem_pv2+Inter_pv2 + Diri_pv2,     zeros(dof_u1,1);
%     -(-elem_pv1+Inter_pv1 + Diri_pv1)',     -(-elem_pv2+Inter_pv2 + Diri_pv2)',    mu^(-1) * Inter_pq,     lambda_p;
%     zeros(1, dof_u1),       zeros(1, dof_u1),       lambda_p',      0];

addDirTerm = 0;

Stokes_stiff = [mu*(elem_u0v0)+Inter_u1v1+addDirTerm*Diri_u1v1,      zeroUU,     -elem_pv1+Inter_pv1 + addDirTerm*Diri_pv1;
    zeroUU,     mu*(elem_u0v0)+Inter_u1v1+addDirTerm*Diri_u1v1,      -elem_pv2+Inter_pv2 + addDirTerm*Diri_pv2;
    -(-elem_pv1+Inter_pv1 + addDirTerm*Diri_pv1)',     -(-elem_pv2+Inter_pv2 + addDirTerm*Diri_pv2)',    mu^(-1) * Inter_pq];


%% get the StokeDarcy system matrix and rhs
Stokes_mass = sparse(2*dof_u1+dof_p, 2*dof_u1+dof_p);
...
for n = 0:NT-1
    ...
    [rhs_f1, rhs_f2] = vecElemRhsMDGTimeDarcy(t, pde.f1, pde.f2, meshInfo, Gaussformulas{1}, degree_u);
    vecRhs_f = [rhs_f1;rhs_f2;zeros(dof_p,1)];
    
    if strcmpi(bd_case,'allDirichlet')
% %         [~,Diri_pv1,Diri_pv2,~,~,~] = ...
% %             DirEdgeAverJumpMDGTimeDarcy(Coeffs_func, pde.gD1, pde.gD2, meshInfo, option, formulaGauss1D, degree_u, degree_p);

        [rhs_pD1, rhs_pD2] = rhsDirMDGTimeDarcy_pD(t, pde.pD, ...
            meshInfo, Gaussformulas{3}, degree_u);
        rhs_DiriBC = [rhs_pD1;rhs_pD2;sparse(dof_p,1)];
        sysRhs = right_term*Uh0 + Dt*(vecRhs_f - rhs_DiriBC);

%         [rhs_gD1, rhs_gD2, rhs_div] = rhsDirMDGTimeDarcy_div(t, Coeffs_func, pde.gD1, pde.gD2, ...
%             meshInfo, option, Gaussformulas{3}, degree_u, degree_p);
%         rhs_DiriBC = [rhs_gD1; rhs_gD2; -rhs_div];
%         sysRhs = right_term*Uh0 + Dt*(vecRhs_f + rhs_DiriBC);
...
if strcmpi(bd_case, 'allDirichlet')
    ...
    DirichletEdgeIndex = meshInfo.bdEdgeIndex; % here, all the bdEdge is set as Diri edge, [Ndir x 1] 

    meshInfo.DirichletEdgeIndex = DirichletEdgeIndex;
    
else
    error('there is no case in getBoundaryInfo ');
