function [rhs_pD1, rhs_pD2] = ...
    rhsDirMDGTimeDarcy_pD(t, func_pD, meshInfo, formulaGauss1D, degreek_u)
%
...
%  
%
%	YcZhang 08/28/2018 -- MM/DD/YYYY
%
%   Last modified 08/28/2018 -- MM/DD/YYYY
%
%
...
rhs_pD1 = zeros(Nelems*Nbases_u,1);
rhs_pD2 = zeros(Nelems*Nbases_u,1);
...
for CurrEdge = 1:length(DirichletEdges)
    ...
    valuefunc_pD = func_pD(t, phyGpointsX, phyGpointsY); % [Npoints x 1]
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    r1 = elemP_nu(1) * u_elemP_Pb' *  (valueCoeff_k11.*valuefunc_pD.*phyGweights);
        %> [NTbases_test x Npoints] * [Npoints x 1] = [NTbases_test x 1].
    rhs_pD1(rhsDirRow,1) = rhs_pD1(rhsDirRow,1) + r1;
    
    r2 = elemP_nu(2) * u_elemP_Pb' *  (valueCoeff_k11.*valuefunc_pD.*phyGweights);
        %> [NTbases_test x Npoints] * [Npoints x 1] = [NTbases_test x 1].
    rhs_pD2(rhsDirRow,1) = rhs_pD2(rhsDirRow,1) + r2;
