function [matElemuv, matElempq, matElemDuDv,matElemPVx,matElemPVy, vecRhsF1,vecRhsF2,vecRhsp,vecLambdaP] = ...
    matElemDuDvMDGTimeDarcy(Coeffs_func,func_f1,func_f2,func_p, meshInfo, formulaGauss2D, degreek_u, degreek_q)
%
...
matElemuv = sparse(Nelems*NTbases_u,Nelems*NTbases_u);
...
vecRhsp = zeros(Nelems*NTbases_q,1);
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    u0v0_temp = zeros(NTbases_u, NTbases_u);
    ...
    for nt = 1:(size(coordTri0Elem,1)/3)
        ...
        rhs_p_temp = rhs_p_temp + q_Pb' * ( phyGweights.*valuefunc_p );
    ...
    vecRhsp(lamRow_vec,1) = vecRhsp(lamRow_vec,1) + rhs_p_temp;
