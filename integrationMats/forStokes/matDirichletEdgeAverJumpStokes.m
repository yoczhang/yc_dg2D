function [matDirichletEdge_uv,matDirichletEdge_qv1,matDirichletEdge_qv2,rhs_gD1,rhs_gD2,rhs_div] = ...
    matDirichletEdgeAverJumpStokes(Coeffs_func, func_gD1, func_gD2, meshInfo, option, formulaGauss1D, degreek_u, degreek_q)
%
...
matDirichletEdge_uv = sparse(Nelems*NTbases_u,Nelems*NTbases_u);
...
%-------------------------------------------------------------------------------------------------------------------%
    ...
    matDirichletEdge_uv = matDirichletEdge_uv ...
    ...
    % get the position in the big mat
    [qv_Row, qv_Col] = getROWandCOL(elemP, 0, NTbases_q, NTbases_u);
    ...
    [rhsdivDirRow, ~] = getROWandCOL(elemP,0,1,NTbases_q);
    r3 = elemP_nu(1) * q_elemP_Pb' * (valueCoeff_k11.*valuefunc_gD1.*phyGweights) ...
        + elemP_nu(2) * q_elemP_Pb' * (valueCoeff_k22.*valuefunc_gD2.*phyGweights);
...
%
    ...
    % {grad_trial}\cdot[test] = 1/2(grad_trial^p + grad_trial^m)\cdot(test^p nu^p + test^m nu^m)
    %AgradtestJtrial_pp = zeros(NTbases_test, NTbases_trial);    
    A1_pp = elemP_nu(1) * elemP_testPb' * bsxfun(@times,phyGaussweights,elemP_trialPb);
        %> [NTbases_test x Npoints] * [Npoints x NTbases_trial] = [NTbases_test x NTbases_trial].
    int_Atrial_Jtest_1 = A1_pp;
        
    
    %-- subPart II, \int_e {(func1,func2)\cdot grad_test}\cdot[trial]
    A2_pp = elemP_nu(2) * elemP_testPb' * bsxfun(@times,phyGaussweights,elemP_trialPb);
        %> [NTbases_test x Npoints] * [Npoints x NTbases_trial] = [NTbases_test x NTbases_trial].
