function [Uh, sysInfo] = dgTransportSolve(meshInfo,pde,option)
%
...
[M, D_elem_dudv, C_elem_udv, rhs0] = ...
    matElemCoeffsDtrialDtestTransport(Coeffs_func,pde.u0,meshInfo,Gaussformulas{1}, trial_k, test_k);
[D_interE, C_interE_upwind] = ...
    matInterEdgeAverJumpTransport(Coeffs_func, meshInfo,option,Gaussformulas{3},trial_k,test_k);

if strcmpi(bd_case,'allDirichlet')
    [D_matDirichletEdge, C_matDirichletEdge] = ...
        matDirichletEdgeAverJumpTransport(Coeffs_func, meshInfo, option, Gaussformulas{3}, trial_k, test_k);
    
    % system matrix
    A = D_elem_dudv + D_interE + D_matDirichletEdge - C_elem_udv + C_interE_upwind + C_matDirichletEdge;
    clear D_elem_dudv D_interE D_matDirichletEdge C_elem_udv C_interE_upwind C_matDirichletEdge
else
    C_matOutflow = ...
        matOutflowEdgeTransport(Coeffs_func, meshInfo, Gaussformulas{3}, trial_k, test_k);
    % system matrix
    A = D_elem_dudv + D_interE - C_elem_udv + C_interE_upwind + C_matOutflow;
...
for n = 0:NT-1
    ...
    rhs_ft = vecElemRhsFtTransport(t, pde.f_rhs, meshInfo, Gaussformulas{1}, test_k);
    
    if strcmpi(bd_case,'allDirichlet')
        rhs_DiriBC = vecEdgeRhsDirichletTransport(t, Coeffs_func, pde.gD, ...
            meshInfo, option, Gaussformulas{3}, test_k);
        Rhs = right_term*r0 + Dt*(rhs_ft + rhs_DiriBC);
    else
        rhs_Inflow = vecInflowEdgeTransport(t, Coeffs_func, meshInfo, pde, Gaussformulas{3}, test_k);
