function [Uh, sysInfo, system] = dgCoupledSDTransportSolve(StokesmeshInfo,DarcymeshInfo,pde,option)
%
...
Ct = pde.c_tau; % this is the coefficient of (P(uh),P(vh))_interface
...
StokesM = [nu*(S_elem_u1v1+S_Inter_u1v1+S_Diri_u1v1)+Ct*interface_u1v1,      nu*(S_elem_u2v1+S_Inter_u2v1+S_Diri_u2v1)+Ct*interface_u2v1,     -S_elem_pv1+S_Inter_pv1+S_Diri_pv1;
    nu*(S_elem_u1v2+S_Inter_u1v2+S_Diri_u1v2)+Ct*interface_u1v2,     nu*(S_elem_u2v2+S_Inter_u2v2+S_Diri_u2v2)+Ct*interface_u2v2,      -S_elem_pv2+S_Inter_pv2+S_Diri_pv2;
...
while(tolerateL2err>1e-10 && iterationStep<=Nstep)
    %% ---- Transport 
    [C_G, C_GxxPlusGyy, C_u1G0xPlusu2G0y, DSC_G0xPlusG0y, C_G_RhsLastStep, Transp_rhs0] = ...
    ...
    [C_diff_K_noAdapE, C_diff_K_AdapE, C_conv_K, DISC_conv_K, C_K_RhsLastStep] = ...
    ...
    [C_K_outflow, DSC_K_outflow, C_K_outflow_RhsLastStep] = ...
    ...
    C_K_inflow = ...
        ...
        formulaGauss1D, degree_phi, degree_u, degree_c); % need to check the code again, 24/10/2017
    
    % system matrix and rhs    
    % have time step
%     systemM = [systemM_SD, zeroUC;
%          - DSC_G0xPlusG0y + DISC_conv_K + DSC_K_outflow, C_G + C_GxxPlusGyy + C_diff_K_AdapE + C_diff_K_noAdapE - C_u1G0xPlusu2G0y + C_conv_K + C_K_outflow];
     
    %--- test have no time
     systemM = [systemM_SD, zeroUC;
         - DSC_G0xPlusG0y + DISC_conv_K + DSC_K_outflow, C_GxxPlusGyy + C_diff_K_AdapE + C_diff_K_noAdapE - C_u1G0xPlusu2G0y + C_conv_K + C_K_outflow];
    
    systemRhs = [systemRhs_SD;
        Transp_rhs0-C_G_RhsLastStep+C_K_RhsLastStep+C_K_outflow_RhsLastStep-C_K_inflow];
    ...
    % solve
    systemUh = systemM\systemRhs;
    
    new_DarcyUh = systemUh(1:dof_phi);
    new_StokesUh =  systemUh(dof_phi+1:dof_phi+2*dof_u1);
    new_Ch = systemUh(dof_SD+1:end);
    % Tolerate Error
    tolerateL2err = getTolerateErrorSDTransport(...
        ...
        degree_phi, degree_u, degree_c);
    tolerateErrInfo = ['the current tolerate Err:' num2str(tolerateL2err)];
    disp(tolerateErrInfo)
    
    % Evaluate
    lastStep_DarcyUh = new_DarcyUh;
    lastStep_StokesUh = new_StokesUh;
    lastStep_Ch = new_Ch;
end
