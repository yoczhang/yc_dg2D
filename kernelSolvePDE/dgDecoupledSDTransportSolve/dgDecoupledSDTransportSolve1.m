function [SD_Uh0, Ch, sysInfo, system] = dgDecoupledSDTransportSolve1(StokesmeshInfo,DarcymeshInfo,pde,option)
%
%   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%   %--------------------------------------------
...
[S_elem_uxvx,S_elem_uxvy,S_elem_uyvx,S_elem_uyvy,S_elem_pv1,S_elem_pv2,S_rhsf1,S_rhsf2,~] = ...
    matElemCoeffsDuDvSDTransport1_TensorStokes(Coeffs_func,pde.f1,pde.f2, StokesmeshInfo,Gaussformulas{1}, degree_u, degree_p);

[S_Inter_u1v1, S_Inter_u1v2, S_Inter_u2v1, S_Inter_u2v2, S_Inter_pv1, S_Inter_pv2] = ...
    matInterEdgeAverJumpSDTransport1_TensorStokes(Coeffs_func, StokesmeshInfo, option, formulaGauss1D, degree_u, degree_p);

[S_Diri_u1v1,S_Diri_u1v2,S_Diri_u2v1,S_Diri_u2v2,S_Diri_pv1,S_Diri_pv2,S_rhs_gD1,S_rhs_gD2,S_rhs_div] = ...
    matDirichletEdgeAverJumpSDTransport1_TensorStokes(Coeffs_func, pde.gD1, pde.gD2, StokesmeshInfo, option, formulaGauss1D, degree_u, degree_p);
...
[interface_u1v1, interface_u2v1, interface_u1v2, interface_u2v2] = ...
    matInterfaceOfStokes_SDTransport1(Coeffs_func, StokesmeshInfo, interfacemeshInfo, formulaGauss1D, degree_u);
...
[Darcy_A, Darcy_rhs_fh] = ...
    matElemCoeffsDtrialDtestSDTransport1_Poisson(Coeffs_func,pde.fp,DarcymeshInfo,Gaussformulas{1}, degree_phi, degree_phi);
Darcy_interE = ...
    matInterEdgeAverJumpSDTransport1_Poisson(Coeffs_func, DarcymeshInfo,option,Gaussformulas{3},degree_phi,degree_phi);
[Darcy_DirichletE, Darcy_rhs_uDterm] = ...
    matDirichletEdgeAverJumpSDTransport1_Poisson(Coeffs_func,pde.gDp,DarcymeshInfo,option,Gaussformulas{3},degree_phi,degree_phi);
% rhsNeumann = vecNeumannEdgePoisson_SDTransport1(Coeffs_func,pde,DarcymeshInfo,Gaussformulas{3},degree_phi);
...
[interface_uphi_n1, interface_uphi_n2, interface_phiu_n1, interface_phiu_n2] = ...
    matSDTransport1_InterfaceStokesDarcy(Coeffs_func, StokesmeshInfo, DarcymeshInfo, interfacemeshInfo, formulaGauss1D, degree_u, degree_phi);
...
% %     getTransportBoundaryInfo(pde, StokesmeshInfo, DarcymeshInfo, ...
...
% %--------------------------------
% h = 1./(sqrt(StokesmeshInfo.Nnodes + DarcymeshInfo.Nnodes)-1);
% if option.theta == 1/2
%     Dt = h^((degree_u+1)/2);
% else
%     Dt = h^(degree_u+1);
% end
% if Dt >= (option.terminalT-option.startingT)
%     Dt = (option.terminalT-option.startingT)/30;
% end
% NT = floor((option.terminalT-option.startingT)/Dt);
% %--------------------------------
disp(['Dt = ',num2str(Dt)])
...
[C_G, C_GxxPlusGyy, C_u1G0xPlusu2G0y, Transp_rhs0] = ...
    matElemCoeffsDtrialDtestSDTransport1_Transport(...
...
[C_diff_K_noAdapInterE, C_diff_K_AdapInterE, C_conv_K_InterE] = ...
	matInterEdgeAverJumpSDTransport1_Transport(...
...
A = C_GxxPlusGyy + C_diff_K_noAdapInterE + C_diff_K_AdapInterE + diff_DiriK ...
...
for n = 0:NT-1
    ...
    rhs_ft = vecElemRhsFtSDTransport1_Transport(t, ...
