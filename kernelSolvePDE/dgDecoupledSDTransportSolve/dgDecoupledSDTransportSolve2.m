function [SD_Uh0, Ch, sysInfo] = dgDecoupledSDTransportSolve2(StokesmeshInfo,DarcymeshInfo,pde,option)
%
%   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%   %--------------------------------------------
...
[S_elem_uxvx,S_elem_uxvy,S_elem_uyvx,S_elem_uyvy,S_elem_pv1,S_elem_pv2,S_rhsf1,S_rhsf2,~] = ...
    matElemCoeffsDuDvSDTransport2_TensorStokes(Coeffs_func,pde.f1,pde.f2, StokesmeshInfo,Gaussformulas{1}, degree_u, degree_p);

[S_Inter_u1v1, S_Inter_u1v2, S_Inter_u2v1, S_Inter_u2v2, S_Inter_pv1, S_Inter_pv2] = ...
    matInterEdgeAverJumpSDTransport2_TensorStokes(Coeffs_func, StokesmeshInfo, option, formulaGauss1D, degree_u, degree_p);

[S_Diri_u1v1,S_Diri_u1v2,S_Diri_u2v1,S_Diri_u2v2,S_Diri_pv1,S_Diri_pv2,S_rhs_gD1,S_rhs_gD2,S_rhs_div] = ...
    matDirichletEdgeAverJumpSDTransport2_TensorStokes(Coeffs_func, pde.gD1, pde.gD2, StokesmeshInfo, option, formulaGauss1D, degree_u, degree_p);
...
[interface_u1v1, interface_u2v1, interface_u1v2, interface_u2v2] = ...
    matInterfaceOfStokes_SDTransport2(Coeffs_func, StokesmeshInfo, interfacemeshInfo, formulaGauss1D, degree_u);

% the Stokes system matrix and rhs
StokesM = [mu*(S_elem_u1v1+S_Inter_u1v1+S_Diri_u1v1)+Kappa*interface_u1v1,      mu*(S_elem_u2v1+S_Inter_u2v1+S_Diri_u2v1)+Kappa*interface_u2v1,     -S_elem_pv1+S_Inter_pv1+S_Diri_pv1;
    mu*(S_elem_u1v2+S_Inter_u1v2+S_Diri_u1v2)+Kappa*interface_u1v2,     mu*(S_elem_u2v2+S_Inter_u2v2+S_Diri_u2v2)+Kappa*interface_u2v2,      -S_elem_pv2+S_Inter_pv2+S_Diri_pv2;
    -(-S_elem_pv1+S_Inter_pv1+S_Diri_pv1)',     -(-S_elem_pv2+S_Inter_pv2+S_Diri_pv2)',     zeroPP];

StokesRhs = [S_rhsf1+mu*S_rhs_gD1;
    S_rhsf2+mu*S_rhs_gD2;
    -S_rhs_div];


%% ---- the Darcy 
[Darcy_A, Darcy_rhs_fh] = ...
    matElemCoeffsDtrialDtestSDTransport2_Poisson(Coeffs_func,pde.fp,DarcymeshInfo,Gaussformulas{1}, degree_phi, degree_phi);
Darcy_interE = ...
    matInterEdgeAverJumpSDTransport2_Poisson(Coeffs_func, DarcymeshInfo,option,Gaussformulas{3},degree_phi,degree_phi);
[Darcy_DirichletE, Darcy_rhs_uDterm] = ...
    matDirichletEdgeAverJumpSDTransport2_Poisson(Coeffs_func,pde.gDp,DarcymeshInfo,option,Gaussformulas{3},degree_phi,degree_phi);
% rhsNeumann = vecNeumannEdgePoisson_SDTransport2(Coeffs_func,pde,DarcymeshInfo,Gaussformulas{3},degree_phi);
...
[interface_uphi_n1, interface_uphi_n2, interface_phiu_n1, interface_phiu_n2] = ...
    matSDTransport2_InterfaceStokesDarcy(Coeffs_func, StokesmeshInfo, DarcymeshInfo, interfacemeshInfo, formulaGauss1D, degree_u, degree_phi);
...
[C_G, C_GxxPlusGyy, C_u1G0xPlusu2G0y, Transp_rhs0] = ...
    matElemCoeffsDtrialDtestSDTransport2_Transport(...
...
[C_diff_K_noAdapE, C_diff_K_AdapE, C_conv_K] = ...
	matInterEdgeAverJumpSDTransport2_Transport(...
...
% %     matDirichletEdgeAverJumpSDTransport1_Transport(...
...
C_K_outflow = ...
	matOutflowEdgeSDTransport2_Transport(pde, ...
...
% % A = C_GxxPlusGyy + C_diff_K_noAdapE + C_diff_K_AdapE + diff_DiriK ...
% %     - C_u1G0xPlusu2G0y + C_conv_K + conv_DiriK;
...
for n = 0:NT-1
    ...
    rhs_ft = vecElemRhsFtSDTransport2_Transport(t, ...
...
% %     sysRhs = rhs_term*Ch0 + Dt*(rhs_ft + rhs_gDt);

    rhs_Inflow = vecInflowEdgeSDTransport2_Transport(t, pde, ...
        DarcymeshInfo, StokesmeshInfo, ...
        lastStep_DarcyUh, lastStep_StokesUh, ...
...
disp(['solve the LOOP system matrix time: ',num2str(sysInfo.SolveSystemTime)])
...
%     StokesmeshInfo.outflowEdgeIndex = S_outflowEdgeIndex;
    ...
    D_interfaceEdgeIndex = D_bdEdgeIndex( abs(DarcymeshInfo.baryEdge(D_bdEdgeIndex,2)-0) <= 5e-7 );
    D_DirichletEdgeIndex = D_bdEdgeIndex( abs(DarcymeshInfo.baryEdge(D_bdEdgeIndex,2)-0) > 5e-7 );
