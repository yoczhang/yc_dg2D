function [Uh, sysInfo, system] = dgNavierStokesSolve1(meshInfo,pde,option)
%
%   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%   %--------------------------------------------
...
[elem_u1v1,elem_nolinearterm_u1v1,elem_pv1,elem_pv2,~,rhsf1,rhsf2,lambda_p] = ...
    matElemCoeffsDtrialDtestNavierStokes1(Coeffs_func, pde.f1, pde.f2, lastStepUh, meshInfo, Gaussformulas{1}, degree_u, degree_p);

[Inter_u1v1, Inter_pv1, Inter_pv2, Inter_nolinear_u1v1] = ...
    matInterEdgeAverJumpNavierStokes1(Coeffs_func, lastStepUh, meshInfo, option, formulaGauss1D, degree_u, degree_p);

[Diri_u1v1,Diri_pv1,Diri_pv2,Diri_nolinear_u1v1,rhs_gD1,rhs_gD2,rhs_div] = ...
    matDirichletEdgeAverJumpNavierStokes1(Coeffs_func, pde.gD1, pde.gD2, lastStepUh, meshInfo, option, formulaGauss1D, degree_u, degree_p);

% % there is no Neumann boudary in the Stokes/Navier-Stokes problem
% % [Krhs_gN1,Krhs_gN2] = ...
% %     vecNeumannEdgeAverJumpNavierStokes1(pde, meshInfo, formulaGauss1D, degree_u);

sysM0 = [nu*(elem_u1v1+elem_nolinearterm_u1v1+Inter_u1v1+Inter_nolinear_u1v1+Diri_u1v1+Diri_nolinear_u1v1),      zeroUU,     -elem_pv1+Inter_pv1+Diri_pv1,     zeros(dof_u1,1);
    zeroUU,     nu*(elem_u1v1+elem_nolinearterm_u1v1+Inter_u1v1+Inter_nolinear_u1v1+Diri_u1v1+Diri_nolinear_u1v1),      -elem_pv2+Inter_pv2+Diri_pv2,     zeros(dof_u1,1);
    -(-elem_pv1+Inter_pv1+Diri_pv1)',     -(-elem_pv2+Inter_pv2+Diri_pv2)',     zeroPP,     lambda_p;
    zeros(1, dof_u1),       zeros(1, dof_u1),       lambda_p',      0];

Rhs0 = [rhsf1+nu*rhs_gD1;
    rhsf2+nu*rhs_gD2;
    -rhs_div;
    0];
...
while(tolerateL2err>1e-10 && iterationStep<=Nstep)
    elem_nolinearterm_u1v1 = matElemCoeffsDtrialDtestNavierStokes1_nolinearTerm(...
        lastStepUh, meshInfo, formulaGauss2D, degree_u);
    Inter_nolinear_u1v1 = matInterEdgeAverJumpNavierStokes1_nolinearTerm(...
        lastStepUh, meshInfo, formulaGauss1D, degree_u);
    Diri_nolinear_u1v1 = matDirichletEdgeAverJumpNavierStokes1_nolinearTerm(...
        lastStepUh, meshInfo, formulaGauss1D, degree_u);
    
    [rhs_nolinear_gD1v,rhs_nolinear_gD2v] = ...
    vecDirichletEdgeAverJumpNavierStokes1_nolinearTerm(pde.gD1, pde.gD2, lastStepUh, ...
    meshInfo, formulaGauss1D, degree_u);
    
    sysM = [nu*(elem_u1v1+elem_nolinearterm_u1v1+Inter_u1v1+Inter_nolinear_u1v1+Diri_u1v1+Diri_nolinear_u1v1),      zeroUU,     -elem_pv1+Inter_pv1+Diri_pv1,     zeros(dof_u1,1);
        zeroUU,     nu*(elem_u1v1+elem_nolinearterm_u1v1+Inter_u1v1+Inter_nolinear_u1v1+Diri_u1v1+Diri_nolinear_u1v1),      -elem_pv2+Inter_pv2+Diri_pv2,     zeros(dof_u1,1);
        -(-elem_pv1+Inter_pv1+Diri_pv1)',     -(-elem_pv2+Inter_pv2+Diri_pv2)',     zeroPP,     lambda_p;
        zeros(1, dof_u1),       zeros(1, dof_u1),       lambda_p',      0];
    Rhs = [rhsf1+nu*rhs_gD1+rhs_nolinear_gD1v;
        rhsf2+nu*rhs_gD2+rhs_nolinear_gD2v;
        -rhs_div;
        0];
